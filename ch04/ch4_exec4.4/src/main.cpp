...
void display(GLFWwindow* window, double currentTime)
{
	...
	glm::vec3 earthOffset(sin((float)currentTime) * 4.0, 0.0f, cos((float)currentTime) * 4.0);
	//// （1）看向太阳
	//vMat = glm::lookAt(glm::vec3(cameraX, cameraY, cameraZ), sunPos, glm::vec3(0.0f, 1.0f, 0.0f));
	// （2）看向地球（跟随地球运动方向）
	glm::vec3 center = sunPos + earthOffset;
	//glm::vec3 eye = sunPos + earthOffset + glm::normalize(earthOffset) * 10.0f;	//要加上f后缀
	glm::vec3 earthOffsetDir = glm::normalize(earthOffset);
	glm::vec3 eyeOffsetDir = glm::cross(glm::vec3(0.0f, 1.0f, 0.0f), earthOffsetDir);
	glm::vec3 eye = sunPos + earthOffset + eyeOffsetDir * 10.0f;
	vMat = glm::lookAt(eye, center, glm::vec3(0.0f, 1.0f, 0.0f));
	mvStack.push(vMat);

	//四棱锥==太阳
	mvStack.push(mvStack.top());
	mvStack.top() *= glm::translate(glm::mat4(1.0f), sunPos);
	mvStack.push(mvStack.top());
	...
	//大立方体==地球
	mvStack.push(mvStack.top());
	mvStack.top() *= glm::translate(glm::mat4(1.0f), earthOffset);
	mvStack.push(mvStack.top());
	mvStack.top() *= glm::rotate(glm::mat4(1.0f), (float)currentTime, glm::vec3(0.0, 1.0, 0.0));
	//// （2）看向地球
	//glm::vec3 earthPos = mvStack.top() * glm::vec4(sunPos, 1.0f);	//可以默认从vec4转换为vec3
	//vMat = glm::lookAt(earthPos+, earthPos, glm::vec3(0.0f, 1.0f, 0.0f));

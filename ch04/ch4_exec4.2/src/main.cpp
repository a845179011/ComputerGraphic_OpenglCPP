...
glm::mat4 buildV(const glm::vec3& U, const glm::vec3& V, const glm::vec3& N, const glm::vec3& cameraPos)
{
	glm::mat4 R{
		U.x, V.x, N.x, 0.0f,
		U.y, V.y, N.y, 0.0f,
...
void display(GLFWwindow* window, double currentTime)
{
	...
	float sqrtVal = (float)std::sqrt(2.0) / 2.0;
	glm::vec3 u{ sqrtVal, 0.0f, -sqrtVal };
	glm::vec3 v{ 0.0f, 1.0f, 0.0f};
	//glm::vec3 u{ 1.0f, 0.0f, 0.0f };
	//glm::vec3 v{ 0.0f, 1.0f, 0.0f };
	glm::vec3 n = glm::cross(u, v);

	vMat = buildV(u, v, n, glm::vec3(cameraX, cameraY, cameraZ));
	mvMat = vMat * mMat;
